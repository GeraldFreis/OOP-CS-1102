...
public:
     ...
     * If the code is invalid, false is returned and the stored code is not 
changed.
    ...
    /**
     * Applies a discount to the Current owed amount, changing its value 
immediately.
     * Reduces the amount by percent argument with values 0.0->1.0. Values must not
be outside this range.
     * If dollars owed would be 20, then a discount percent of 0.25 would have 
...
private:
    /**
     * Returns true if the given invoiceCode is a 4 character, uppercase string, 
false otherwise.
     */
